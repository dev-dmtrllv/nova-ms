...
public class Quest {
	...
	protected Map<QuestRequirementType, AbstractQuestRequirement> completeReqs = new EnumMap<>(
			QuestRequirementType.class);
	...
	public boolean canStartQuestByStatus(Character chr) {
		...
		return !(!mqs.getStatus().equals(Status.NOT_STARTED)
				&& !(mqs.getStatus().equals(Status.COMPLETED) && repeatable));
	}
	...
	public boolean forceComplete(Character chr, int npc) {
		...
		chr.getMap().broadcastMessage(chr, PacketCreator.showForeignEffect(chr.getId(), 9), false); // use 9 instead of
																									// 12 for both
		return true;
	}
	...
	private AbstractQuestRequirement getRequirement(QuestRequirementType type, Data data) {
		...
		switch (type) {
			...
			case END:
				break;
			default:
				// FilePrinter.printError(FilePrinter.EXCEPTION_CAUGHT, "Unhandled Requirement
				// Type: " + type.toString() + " QuestID: " + this.getId());
				break;
		}
		return ret;
	}
	...
	private AbstractQuestAction getAction(QuestActionType type, Data data) {
		...
		switch (type) {
			...
			case INFO:
				...
				break;
			default:
				// FilePrinter.printError(FilePrinter.EXCEPTION_CAUGHT, "Unhandled Action Type:
				// " + type.toString() + " QuestID: " + this.getId());
				break;
		}
		return ret;
	}
