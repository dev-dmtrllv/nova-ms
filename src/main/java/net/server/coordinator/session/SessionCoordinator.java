...
public class SessionCoordinator {
			...
			for (HwidRelevance hwidRelevance : hwidRelevances) {
				if (hwidRelevance.hwid().endsWith(hwid.hwid())) {
					if (!routineCheck) {
						...
						SessionDAO.updateAccountAccess(con, hwid, accountId, expiry,
								hwidRelevance.getIncrementedRelevance());
					}
	...
	/**
	 * Overwrites any existing online client for the account id, making sure to
	...
	private void disconnectClientIfOnline(int accountId) {
		...
		if (ingameClient != null) { // thanks MedicOP for finding out a loss of loggedin account uniqueness when
									// using the CMS "Unstuck" feature
	...
	public AntiMulticlientResult attemptGameSession(Client client, int accountId, Hwid hwid) {
		...
		if (!YamlConfig.config.server.DETERRED_MULTICLIENT) {
			...
			hostHwidCache.addEntry(client.getRemoteAddress(), hwid); // no HWID information on the loggedin newcomer
																		// session...
			return AntiMulticlientResult.SUCCESS;
		}
		...
		try {
			...
			// assumption: after a SUCCESSFUL login attempt, the incoming client WILL
			// receive a new IoSession from the game server

			// updated session CLIENT_HWID attribute will be set when the player log in the
			// game
			onlineRemoteHwids.add(hwid);
			hostHwidCache.addEntry(remoteHost, hwid);
	...
	public Hwid pickLoginSessionHwid(Client client) {
		...
		// thanks BHB, resinate for noticing players from same network not being able to
		// login
